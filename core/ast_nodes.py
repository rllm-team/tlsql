...
class InExpr(Expr):
    ...
    Attributes:
        column: Column reference expression 
        values: Value list (expression list) 
    """
    
    column: Expr        # Column reference 
    values: List[Expr]  # Value list 
...
class ColumnSelector(ASTNode):
    ...
    Supported formats:
    - table.column: Single column 
    - table.*: All columns of table
...
class PredictType(ASTNode):
    ...
    type_name: str  # 'CLF' or 'REG' 
    
    @property
    def is_classifier(self) -> bool:
        """Determine if it's a classifier """
        return self.type_name.upper() == 'CLF'
    
    @property
    def is_regressor(self) -> bool:
        """Determine if it's a regressor """
        return self.type_name.upper() == 'REG'


@dataclass
class ValueClause(ASTNode):
    """Represents VALUE clause in PREDICT statement 
    ...
    target: ColumnReference        # Prediction target column 
    predict_type: PredictType      # Prediction type 
